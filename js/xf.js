...
(function(window){
	...
	*/
	XF.start = function(options) {
		...
		createRouter(options.router.routes, options.router.handlers);
...
/*
	...
	/**
		...
		@private
	*/
	var createRouter = function(routes, handlers) {
		...
		if(XF.Router) {
			...
			XF.Router = new XF.RouterClass(routes, handlers);
		}
	};
	...
	*/
	var placeAnchorHooks = function() {
		$('[data-href]').live('click',function() {
			XF.Router.navigate( $(this).attr('data-href'), true );
	...
	*/
	XF.HistoryClass = function() {
		...
		*/
		...
		/**
			Indicates whether the history handling has already been started
			@type {Boolean}
			@private
		*/
	...
	_.extend(XF.HistoryClass.prototype, /** @lends XF.HistoryClass# */ {
		...
		*/
		...
		/**
			Property to store previous route
		*/
		prevFragment: '',
		...
		*/
		start : function(options) {
			...
			if (this.historyStarted) throw new Error('XF.History has already been started');
		...
		*/
		loadUrl : function(fragmentOverride) {
			...
			XF.Utils.AddressBar.hide();
			XF.Utils.AddressBar.hideOnLoad();
			return matched;
		},
		...
		*/
		...
		/**
			Navigates to previous page in history if possible
		*/
		goBack : function() {
			if(window.history && window.history.back instanceof Function) {
				window.history.back();
				XF.Utils.AddressBar.hide();
				XF.Utils.AddressBar.hideOnLoad();
			}
	...
	*/
	XF.RouterClass = function(routes, handlers) {
		/**
			Contains all routes definitions
			@type {Object}
		*/
		this.routes = routes || {};
		...
		*/
		...
		/**
			Saves the latest route dispatched
			@type Object
			@private
		*/
		this.mostRecentCalled = null;
	...
	_.extend(XF.RouterClass.prototype, XF.Events, /** @lends XF.RouterClass.prototype */{
		...
		*/
		route : function(route, name, callback) {
			XF.History || (XF.History = new XF.HistoryClass);
			
			if (!_.isRegExp(route)) route = this.routeToRegExp(route);
			XF.History.route(
				route,
				_.bind(
					function(fragment) {
						var args = this.extractParameters(route, fragment);
		...
		/**
			Simple proxy to 'XF.History' to save a fragment into the history.
			@params {Object} fragment
			@params {Object} triggerRoute
		*/
		navigate : function(fragment, triggerRoute) {
			XF.History.navigate(fragment, triggerRoute);
		},
		...
		*/
		...
		/**
			...
			@param {RegExp} route route pattern
			@param {Object} fragment
			@return {String}
			@private
		*/
		...
		/**
			Initializes routers, creates handlers and proxy handlers to track {@link XF.RouterClass#mostRecentCalled}
			@private
		*/
		initialize : function() {
			...
			// adding 'notFound' route
			this.route(new RegExp('.*', 'g'), 'notFound', function() {
				var callback = $this['notFound'];
				if(callback) {
					callback.apply($this);
				}
			});
			...
			// adding rest routes
			$.each(this.routes, function(routeDef, routeName) {
				var proxyCallback = function() {
					var callback = $this.handlers[routeName];
					if(callback) {
						callback.apply($this.handlers, arguments);
					}
					
					$this.mostRecentCalled = {
						name: routeName,
						args: arguments
					};
				};
				$this.route(routeDef, routeName, proxyCallback);
			});
		...
		*/
		...
		/**
			Binds a callback to approppriate route
			@param {String} routeName Route name
			@param {Function} callback A function to be called when route is visited
		*/
		bindRoute : function(routeName, callback) {
			this.bind('route:' + routeName, callback);
			
			if(this.mostRecentCalled && this.mostRecentCalled.name == routeName) {
				callback(this.mostRecentCalled.args);
			}
		},
